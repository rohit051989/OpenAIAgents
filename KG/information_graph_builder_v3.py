...
from call_hierarchy_extension import ClassInfo, JavaCallHierarchyParser
from neo4j_direct_step_loader_v2 import JobDef, parse_directory
from call_hierarchy_extension_v2 import BeanDef, SpringBeanRegistry, enrich_with_call_hierarchy_v2
    ...
    def process_class_recursive(class_fqn: str, depth: int = 0, max_depth: int = 10):
        ...
        for method_name, method_def in class_info.methods.items():
            ...
            if method_fqn not in processed_methods:
                ...
                # Add database operations if present
                if method_def.db_operations:
                    cypher_stmt += f", m.dbOperationCount = {len(method_def.db_operations)}"
                    # Serialize db operations details
                    db_ops_details = []
                    for op in method_def.db_operations:
                        op_str = f"{op.operation_type}:{op.table_name or 'UNKNOWN'}:{op.confidence}"
                        db_ops_details.append(escape_cypher_string(op_str))
                    cypher_stmt += f", m.dbOperations = ['{"','".join(db_ops_details)}']"
                else:
                    cypher_stmt += ", m.dbOperationCount = 0"
                
                # Add procedure calls if present
                if method_def.procedure_calls:
                    cypher_stmt += f", m.procedureCallCount = {len(method_def.procedure_calls)}"
                    # Serialize procedure call details
                    proc_details = []
                    for proc in method_def.procedure_calls:
                        proc_str = f"{proc.procedure_name}:{proc.schema or 'UNKNOWN'}"
                        proc_details.append(escape_cypher_string(proc_str))
                    cypher_stmt += f", m.procedureCalls = ['{"','".join(proc_details)}']"
                else:
                    cypher_stmt += ", m.procedureCallCount = 0"
                
                # Add shell executions if present
                if method_def.shell_executions:
                    cypher_stmt += f", m.shellExecutionCount = {len(method_def.shell_executions)}"
                    # Serialize shell execution details
                    shell_details = []
                    for shell in method_def.shell_executions:
                        shell_str = f"{shell.script_type}:{shell.command or 'UNKNOWN'}"
                        shell_details.append(escape_cypher_string(shell_str))
                    cypher_stmt += f", m.shellExecutions = ['{"','".join(shell_details)}']"
                else:
                ...
                # Create Resource nodes and relationships for database operations
                if method_def.db_operations:
                    for op in method_def.db_operations:
                        table_name = op.table_name.upper() if op.table_name else "UNKNOWN"
                        escaped_table_name = escape_cypher_string(table_name)
                        escaped_entity_type = escape_cypher_string(op.entity_type) if op.entity_type else "UNKNOWN"
                        
                        # Match existing Resource by name and type, or create new one
                        # Generate unique ID only when creating new resource
                        import uuid
                        unique_id = f"RES_TABLE_{uuid.uuid4().hex[:8].upper()}"
                        escaped_resource_id = escape_cypher_string(unique_id)
                        
                        lines.append(
                            f"MERGE (r:Resource {{name: '{escaped_table_name}', type: 'TABLE'}}) "
                            f"ON CREATE SET r.id = '{escaped_resource_id}', "
                            f"r.enabled = true, "
                            f"r.schemaName = '{escaped_entity_type}' "
                            f"ON MATCH SET r.schemaName = COALESCE(r.schemaName, '{escaped_entity_type}');"
                        )
                        
                        # Create relationship between JavaMethod and Resource
                        lines.append(
                            f"MATCH (m:JavaMethod {{fqn: '{escaped_method_fqn}'}}) "
                            f"MATCH (r:Resource {{name: '{escaped_table_name}', type: 'TABLE'}}) "
                            f"MERGE (m)-[:DB_OPERATION {{operationType: '{op.operation_type}', confidence: '{op.confidence}'}}]->(r);"
                        )
                ...
                for call in method_def.calls:
                    if call.target_class:
                        called_class_fqn = call.target_class
                        ...
                        if call_key not in processed_method_calls:
                            ...
                            if called_class_fqn in all_classes_cache:
                                ...
                                if call.method_name in called_class_info.methods:
                                    ...
                                    # Add database operations if present
                                    if called_method_def.db_operations:
                                        called_cypher_stmt += f", cm.dbOperationCount = {len(called_method_def.db_operations)}"
                                        # Serialize db operations details
                                        db_ops_details = []
                                        for op in called_method_def.db_operations:
                                            op_str = f"{op.operation_type}:{op.table_name or 'UNKNOWN'}:{op.confidence}"
                                            db_ops_details.append(escape_cypher_string(op_str))
                                        called_cypher_stmt += f", cm.dbOperations = ['{"','".join(db_ops_details)}']"
                                    else:
                                        called_cypher_stmt += ", cm.dbOperationCount = 0"
                                    
                                    # Add procedure calls if present
                                    if called_method_def.procedure_calls:
                                        called_cypher_stmt += f", cm.procedureCallCount = {len(called_method_def.procedure_calls)}"
                                        # Serialize procedure call details
                                        proc_details = []
                                        for proc in called_method_def.procedure_calls:
                                            proc_str = f"{proc.procedure_name}:{proc.schema or 'UNKNOWN'}"
                                            proc_details.append(escape_cypher_string(proc_str))
                                        called_cypher_stmt += f", cm.procedureCalls = ['{"','".join(proc_details)}']"
                                    else:
                                        called_cypher_stmt += ", cm.procedureCallCount = 0"
                                    
                                    # Add shell executions if present
                                    if called_method_def.shell_executions:
                                        called_cypher_stmt += f", cm.shellExecutionCount = {len(called_method_def.shell_executions)}"
                                        # Serialize shell execution details
                                        shell_details = []
                                        for shell in called_method_def.shell_executions:
                                            shell_str = f"{shell.script_type}:{shell.command or 'UNKNOWN'}"
                                            shell_details.append(escape_cypher_string(shell_str))
                                        called_cypher_stmt += f", cm.shellExecutions = ['{"','".join(shell_details)}']"
                                    else:
                                    ...
                                    if called_method_def.db_operations:
                                        for op in called_method_def.db_operations:
                                            table_name = op.table_name if op.table_name else "UNKNOWN"
                                            escaped_table_name = escape_cypher_string(table_name)
                                            escaped_entity_type = escape_cypher_string(op.entity_type) if op.entity_type else "UNKNOWN"
                                            ...
                                            lines.append(
                                                ...
                                                f"MATCH (r:Resource {{id: '{escaped_resource_id}'}}) "
                                                f"MERGE (cm)-[:DB_OPERATION {{operationType: '{op.operation_type}', confidence: '{op.confidence}'}}]->(r);"
                                            )
...
class InformationGraphBuilder:
    ...
    def _load_classes(self):
        ...
        # Step 7: Consolidate database operations at Step level
    ...
    def _consolidate_step_db_operations(self):
        ...
        This method replaces the APOC query:
        - Finds entry methods for each Step (execute for TASKLET, read/process/write for CHUNK)
        - Uses BFS to traverse CALLS relationships
        - Collects all dbOperations from reachable JavaMethod nodes
        - Sets stepDbOperations and stepDbOperationCount on Step nodes
        """
        print("\n" + "=" * 80)
        print("Consolidating Database Operations at Step Level")
        print("=" * 80)
        
        # Step 1: Get all Steps with their implementation classes
        query_steps = """
        MATCH (s:Step)
        ...
        for step_data in steps_data:
            ...
            # Step 2: Find entry methods for this step
            query_entry_methods = """
            MATCH (s:Step)
            ...
            # Step 3: For each entry method, perform BFS to collect all db operations
            all_db_operations = set()
            
            for entry_method in entry_methods:
                method_id = entry_method['methodId']
                
                # Add operations from entry method itself if present
                ...
                while queue:
                    ...
                    # Find all methods called by current method
                    query_calls = """
